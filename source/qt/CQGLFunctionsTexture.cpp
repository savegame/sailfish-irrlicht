...
CQGLFunctionsTexture::CQGLFunctionsTexture(const irr::io::path &name, const core::array<IImage *> &image, E_TEXTURE_TYPE type, CQGLFunctionsDriver *driver) : ITexture(name, type), Driver(driver), TextureType(GL_TEXTURE_2D),
    ...
    KeepImage(false), AutoGenerateMipMaps(false)
{
	...
	Driver->m_functions->glGenTextures(1, &TextureName);
	
	const CQGLFunctionsTexture* prevTexture = Driver->getCacheHandler()->getTextureCache().get(0);
	Driver->getCacheHandler()->getTextureCache().set(0, this);
	
	Driver->m_functions->glTexParameteri(TextureType, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	Driver->m_functions->glTexParameteri(TextureType, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	
	if (HasMipMaps && AutoGenerateMipMaps)
	{
		if (Driver->getTextureCreationFlag(ETCF_OPTIMIZED_FOR_SPEED))
			Driver->m_functions->glHint(GL_GENERATE_MIPMAP_HINT, GL_FASTEST);
		else if (Driver->getTextureCreationFlag(ETCF_OPTIMIZED_FOR_QUALITY))
			Driver->m_functions->glHint(GL_GENERATE_MIPMAP_HINT, GL_NICEST);
		else
...
CQGLFunctionsTexture::CQGLFunctionsTexture(const irr::io::path &name, const core::dimension2d<irr::u32> &size, ECOLOR_FORMAT format, CQGLFunctionsDriver *driver) : ITexture(name, ETT_2D), Driver(driver), TextureType(GL_TEXTURE_2D),
    ...
    AutoGenerateMipMaps(false)
{
	...
	Driver->m_functions->glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);
	Driver->m_functions->glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);
	Driver->m_functions->glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_S, GL_CLAMP_TO_EDGE);
	Driver->m_functions->glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP_TO_EDGE);
	
#if defined(GL_VERSION_1_2)
	Driver->m_functions->glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_R, GL_CLAMP_TO_EDGE);
#endif
	...
	Driver->m_functions->glTexImage2D(GL_TEXTURE_2D, 0, InternalFormat, Size.Width, Size.Height, 0, PixelFormat, PixelType, 0);
	
	Driver->getCacheHandler()->getTextureCache().set(0, prevTexture);
	Driver->testGLError(__LINE__);
}

CQGLFunctionsTexture::~CQGLFunctionsTexture()
{
	Driver->getCacheHandler()->getTextureCache().remove(this);
	
	if (TextureName)
		Driver->m_functions->glDeleteTextures(1, &TextureName);
...
void *CQGLFunctionsTexture::lock(E_TEXTURE_LOCK_MODE mode, u32 layer)
{
	...
	else
	{
		...
		if (LockImage && mode != ETLM_WRITE_ONLY)
		{
			...
			CQGLFunctionsTexture* tmpTexture = new CQGLFunctionsTexture("OGL_CORE_LOCK_TEXTURE", Size, ColorFormat, Driver);
			...
			Driver->m_functions->glClear(GL_COLOR_BUFFER_BIT);
			
			Driver->draw2DImage(this, layer, true);
			
			Driver->m_functions->glReadPixels(0, 0, Size.Width, Size.Height, GL_RGBA, GL_UNSIGNED_BYTE, tmpImage->getData());
...
void CQGLFunctionsTexture::unlock()
{
	if (!LockImage)
		return;
...
void CQGLFunctionsTexture::regenerateMipMapLevels(void *data, u32 layer)
{
...
GLenum CQGLFunctionsTexture::getOpenGLTextureType() const
{
	return TextureType;
}

GLuint CQGLFunctionsTexture::getOpenGLTextureName() const
{
	return TextureName;
}

CQGLFunctionsTexture::SStatesCache &CQGLFunctionsTexture::getStatesCache() const
{
	return StatesCache;
}

ECOLOR_FORMAT CQGLFunctionsTexture::getBestColorFormat(ECOLOR_FORMAT format)
{
...
void CQGLFunctionsTexture::getImageValues(const IImage *image)
{
...
void CQGLFunctionsTexture::uploadTexture(bool initTexture, u32 layer, u32 level, void *data)
{
	if (!data)
		return;
	...
	if (!IImage::isCompressedFormat(ColorFormat))
	{
		...
		case GL_TEXTURE_CUBE_MAP:
			if (initTexture)
				Driver->m_functions->glTexImage2D(tmpTextureType, level, InternalFormat, width, height, 0, PixelFormat, PixelType, tmpData);
			else
				Driver->m_functions->glTexSubImage2D(tmpTextureType, level, 0, 0, width, height, PixelFormat, PixelType, tmpData);
