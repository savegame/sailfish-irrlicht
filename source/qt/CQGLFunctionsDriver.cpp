...
void CQGLFunctionsDriver::initExtensions()
{
	Version = 0;
	s32 multiplier = 100;
	...
	{
		m_functions->glGetIntegerv(GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT, &val);
...
#endif
	m_functions->glGetFloatv(GL_ALIASED_LINE_WIDTH_RANGE, DimAliasedLine);
	m_functions->glGetFloatv(GL_ALIASED_POINT_SIZE_RANGE, DimAliasedPoint);

	Feature.TextureUnit = core::min_(Feature.TextureUnit, static_cast<u8>(MATERIAL_MAX_TEXTURES));
	Feature.ColorAttachment = 1;
}

void CQGLFunctionsDriver::dump() const
{
