...
CQGLFunctionsRenderTarget::CQGLFunctionsRenderTarget(CQGLFunctionsDriver *driver) 
    : AssignedDepth(false)
...
{
#ifdef _DEBUG
	setDebugName("CQGLFunctionsRenderTarget");
#endif
...
CQGLFunctionsRenderTarget::~CQGLFunctionsRenderTarget()
{
...
void CQGLFunctionsRenderTarget::setTexture(const core::array<ITexture*>& texture, ITexture* depthStencil)
{
	...
	if (textureUpdate || depthStencilUpdate)
	{
		...
		if (textureUpdate)
		{
			...
			for (u32 i = 0; i < Texture.size(); ++i)
			{
				CQGLFunctionsTexture* currentTexture = (texture[i] && texture[i]->getDriverType() == DriverType) ? static_cast<CQGLFunctionsTexture*>(texture[i]) : 0;
		...
		{
			CQGLFunctionsTexture* currentTexture = (depthStencil && depthStencil->getDriverType() == DriverType) ? static_cast<CQGLFunctionsTexture*>(depthStencil) : 0;
...
void CQGLFunctionsRenderTarget::update()
{
	if (RequestTextureUpdate || RequestDepthStencilUpdate)
	{
		...
		if (RequestTextureUpdate)
		{
			...
			{
				GLuint textureID = (Texture[i]) ? static_cast<CQGLFunctionsTexture*>(Texture[i])->getOpenGLTextureName() : 0;
		...
		if (RequestDepthStencilUpdate)
		{
			...
			if (IImage::isDepthFormat(textureFormat))
			{
				GLuint textureID = static_cast<CQGLFunctionsTexture*>(DepthStencil)->getOpenGLTextureName();
...
GLuint CQGLFunctionsRenderTarget::getBufferID() const
{
	return BufferID;
}

bool CQGLFunctionsRenderTarget::checkFBO(CQGLFunctionsDriver *driver)
{
	if (ColorAttachment == 0)
		return true;
